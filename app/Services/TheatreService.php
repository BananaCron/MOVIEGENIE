...
class TheatreService
{
    ...
    // Method to get opening movies, optionally filtered by country ID
    public function getOpeningMovies($countryId = null)
    {
        // Make a GET request to the API endpoint with optional country ID query parameter
        $response = $this->client->request('GET', 'https://flixster.p.rapidapi.com/movies/get-opening', [
            'headers' => $this->headers,
            'query' => [
                'countryId' => $countryId,
                // Country IDs are listed in the comment below
